...
Pixel BinaryFilter::WHITE = Pixel(255,255,255);
Pixel BinaryFilter::BLACK = Pixel(0,0,0);
...
: PixelFilter("Binary Filter"), primary(WHITE), secondary(BLACK) {}
...
: PixelFilter("Binary Filter"), primary(primary), secondary(secondary) {}
...
Pixel BinaryFilter::apply_transform(const Pixel& p) const {
  ...
  if(lum >= 255/2) {
    return Pixel(primary.r(),primary.g(),primary.b());
  }

  return Pixel(secondary.r(),secondary.g(),secondary.b());
}
